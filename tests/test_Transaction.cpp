...
{//создаём константы. base_fee для полноценного теста должен быть хотя бы 51.
	const int base_A = 5000; 
	const int base_B = 5000;
...
//проверяем что если входные параметры правильные, то транзакция проходит
std::cout << "3 " << Bob.GetBalance() << " " << Alice.GetBalance() <<  "\n";
	ASSERT_EQ(test_tran.Make(Alice, Bob, 1000), true);
	std::cout << "4 " << Bob.GetBalance() << " " << Alice.GetBalance() <<  "\n";
